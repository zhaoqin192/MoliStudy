...
- (void) leftArrowTapped
{
    ...
    if (indexPath.item < [self.dataSource numberOfItemsInPickerView:self] - 1)
    {
        [self selectItem:indexPath.item + 1 animated:YES];
    }
}
...
- (id)initWithFrame:(CGRect)frame
{
	...
	if (self) {
		[self initialize];
        self.leftArrow = [[PickerLeftArrow alloc] initWithFrame:CGRectMake(0, 0, frame.size.height, frame.size.height)];
        self.leftArrow.backgroundColor = [UIColor colorWithRed:(153.0f/255.0f) green:(204.0f/255.0f) blue:(102.0f/255.0f) alpha:1.0f];
        [self addSubview:self.leftArrow];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(leftArrowTapped) name:@"PickerLeftArrowTapped" object:nil];
        
        self.rightArrow = [[PickerRightArrow alloc] initWithFrame:CGRectMake(frame.size.width - frame.size.height, 0, frame.size.height, frame.size.height)];
        self.rightArrow.backgroundColor = [UIColor clearColor];
        [self addSubview:self.rightArrow];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(rightArrowTapped) name:@"PickerRightArrowTapped" object:nil];
	}
	return self;
}
